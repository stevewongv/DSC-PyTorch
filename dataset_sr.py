...
class TrainValDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Resize images
        image_lab = cv2.resize(image_lab, (256, 256))
        label_lab = cv2.resize(label_lab, (256, 256))
        ...
        # label_lab shape:  (1, 256, 256, 3)
        # image_nom shape:  torch.Size([3, 256, 256])
        # align the shape of label_lab to image_nom
        label_lab = label_lab.transpose(3, 0, 1, 2)
        # label_lab shape:  (3, 1, 256, 256)
        # align the shape of label_lab to image_nom
        label_lab = np.squeeze(label_lab)
        # print("label_lab shape: ", label_lab.shape)

        sample = {'O': image_nom, 'B': label_lab, 'image': np.array(image_lab, dtype='float32').transpose(2, 0, 1) / 255}
        return sample
